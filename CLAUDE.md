...
- Release/version rules are shared with other agents: update `CHANGELOG.md`, follow `docs/release.md`, and keep the three desktop version files in sync for releases. A push to `main` with a new desktop version auto-creates the release tag and publishes the DMG after CI passes.
